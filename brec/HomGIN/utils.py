...
# from utils_ids import subgraph_counts2ids, homcount_counts2ids
from utils_ids import homcount_counts2ids
...
def process_arguments(args):
    ...
    extract_id_fn = homcount_counts2ids    
    ...
    return args, extract_id_fn, loss_fn, prediction_fn, perf_opt        
...
def prepare_dataset(path, 
    ...
    if maybe_load:
        ...
        if os.path.exists(data_file):  # load
            graphs_ptg, num_classes = load_dataset(data_file)
            loaded = True
    ...
    if not loaded:
        ...
        graphs_ptg, num_classes, num_node_type, num_edge_type = generate_dataset(path, name, extract_ids_fn, regression)
        if data_file is not None:
            print("Saving dataset to {}".format(data_file))
            torch.save((graphs_ptg, num_classes), data_file)

        if num_node_type is not None:
            torch.save((num_node_type, num_edge_type), os.path.join(path, 'processed', 'num_feature_types.pt'))

    return graphs_ptg, num_classes
